...
@pytest.fixture
def mock_env_utils(monkeypatch):
...
class TestPDFProcessorS3Routing:
    ...
    @patch('boto3.client')
    def test_s3_input_derives_s3_output(self, mock_boto3_client, temp_pdf, mock_env_utils):
        ...
        # Simulate S3 source - this is testing the derivation logic
        s_input = os.environ.get("S3_INPUT_PDF_PATH")
        expected_output = s_input.replace("/input_data/", "/output_data/", 1)

        assert expected_output == 's3://test-bucket/output_data/pdf'

        # Test upload with S3 target
        from backend.services.pdf_processor import _maybe_upload_output_to_s3
        result = _maybe_upload_output_to_s3(str(temp_pdf), expected_output)
        assert result.startswith('s3://test-bucket/output_data/pdf/')
        mock_s3.upload_file.assert_called_once()

    @patch('boto3.client')
    def test_no_upload_if_not_s3(self, mock_boto3_client, temp_pdf, mock_env_utils):
        """Test that no upload happens when target is not S3."""
        # Test with local path (not S3)
        from backend.services.pdf_processor import _maybe_upload_output_to_s3
        result = _maybe_upload_output_to_s3(str(temp_pdf), '/local/path/output')
        assert result is None  # Since no S3 target

    @patch('boto3.client')
    def test_explicit_s3_env_output(self, mock_boto3_client, temp_pdf, mock_env_utils):
        ...
        processor = MasterPDFProcessor()
        # The processor should use the explicit env var
        expected_target = 's3://explicit-bucket/explicit-prefix'

        from backend.services.pdf_processor import _maybe_upload_output_to_s3
        result = _maybe_upload_output_to_s3(str(temp_pdf), expected_target)
